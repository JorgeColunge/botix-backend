...
async function sendWhatsAppLocation(io, phone, lat, lng, conversationId) {
  ...
  try {
    ...
    io.emit('newMessage', {
      ...
      latitude: lat,
      longitude: lng,
      type_message_in_conversation: 'reply',
      unread_messages: unreadMessages,
      responsibleUserId: responsibleUserId
    });
    console.log('Mensaje emitido:', newMessage.replies_id);
...
async function obtenerRespuestaGPT(prompt) {
  const apiKey = process.env.OPENAI_API_KEY; // Asegúrate de tener tu API Key en las variables de entorno
        ...
        case 'awaiting_response':
            ...
            case 'awaiting_location':
                ...
                if (message.type === 'location') {
                  ...
                  responseText = `Confirmas que la ubicación coincide con la dirección que nos proporsionaste? Si es correcto, responde 'Sí'. Si no, intenta compartiendo tu ubicación siguiendo el instructivo que te comparti anteriormente.`;
                  await sendWhatsAppMessage(io, senderId, responseText, conversationId);
            ...
            case 'awaiting_correct_address':
                // Aquí puedes usar el texto enviado por el usuario como la dirección confirmada
