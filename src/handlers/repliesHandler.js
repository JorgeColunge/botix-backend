    ...
    for (const contact of contacts) {
      try {
        ...
        if (template.header_type === 'TEXT') {
          ...
          const videoUrl = `${backendUrl}${template.medio}`;
          response = await sendVideoWhatsAppMessage(contact.phone_number, template.nombre, template.language, `${backendUrl}${template.medio}?token=${newToken}`, parameters.body, whatsapp_api_token, whatsapp_phone_number_id, whatsapp_business_account_id);
          mediaUrl = videoUrl;
          ...
          response = await sendDocumentWhatsAppMessage(contact.phone_number, template.nombre, template.language, mediaId, parameters.body, whatsapp_api_token, whatsapp_phone_number_id, whatsapp_business_account_id);
          mediaUrl = documentUrl;
  ...
  if (conversation.conversation_id) {
    try {
      ...
      if (template.header_type === 'TEXT') {
        ...
        const imageUrl = template.medio.startsWith("http") ? `${template.medio}` : `${backendUrl}${template.medio}?token=${newToken}`
        response = await sendImageWhatsAppMessage(phoneNumber, template.nombre, template.language, `${imageUrl}`, parametersReal.body, whatsapp_api_token, whatsapp_phone_number_id, whatsapp_business_account_id);
        mediaUrl = imageUrl;
      } else if (template.header_type === 'VIDEO') {
        const videoUrl = template.medio.startsWith("http") ? `${template.medio}` : `${backendUrl}${template.medio}?token=${newToken}`
        response = await sendVideoWhatsAppMessage(phoneNumber, template.nombre, template.language, `${videoUrl}`, parametersReal.body, whatsapp_api_token, whatsapp_phone_number_id, whatsapp_business_account_id);
        mediaUrl = videoUrl;
      } else if (template.header_type === 'DOCUMENT') {
        const documentUrl = template.medio.startsWith("http") ? `${template.medio}` : `${backendUrl}${template.medio}?token=${newToken}`
        // const mediaId = await uploadDocumentToWhatsApp(`${backendUrl}${template.medio}?token=${newToken}`, whatsapp_api_token);
        response = await sendDocumentWhatsAppMessage(phoneNumber, template.nombre, template.language, `${documentUrl}`, parametersReal.body, whatsapp_api_token, whatsapp_phone_number_id, whatsapp_business_account_id);
        mediaUrl = documentUrl;
      } else {
        response = await sendWhatsAppMessageCampaing(conversation.phone_number,
          template, // Enviar el template completo
