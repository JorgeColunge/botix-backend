...
app.post('/create-template', async (req, res) => {
  ...
  components.forEach(component => {
    if (component.type === 'HEADER') {
      ...
      if (['IMAGE', 'VIDEO', 'DOCUMENT'].includes(headerType)) {
        ...
        headerExample = { "header_text": ["Ejemplo de encabezado"] }; // Añadir ejemplo para HEADER
      }
    } else if (component.type === 'BODY') {
      bodyText = component.text;
      bodyExample = { "body_text": [["Ejemplo de cuerpo"]] }; // Añadir ejemplo para BODY
  ...
  if (isMediaTemplate) {
    try {
      const query = `
        INSERT INTO templates_wa (type, nombre, language, header_type, type_medio, medio, body_text, header_text, footer, buttons, state, company_id)
